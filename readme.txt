...
拡張子は固定です。デフォルトでは jpg に設定サれていますが、main.js の上部にある const ext を書き換えることで png なども使えます。
横長画像は x フォルダ、縦長画像は y に放り込んでください。
その際、名前は番号にしてください（例：1.jpg, 2.jpg ...）。
const xImages と const yImages にそれぞれのフォルダに入れた画像の枚数を入力してください。
